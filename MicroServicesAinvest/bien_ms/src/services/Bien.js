import { Bien } from "../models/Bien.js"; // add validate
...
if (queryparam["Date mutation"]) {
  ...
  query["$expr"] = {
    ...
    if (queryparam["Valeur fonciere max"] && queryparam["Valeur fonciere min"]) {
      const valueMin = parseInt(queryparam["Valeur fonciere min"]);
      const valueMax = parseInt(queryparam["Valeur fonciere max"]);
      query["Valeur fonciere"] = { $gte: valueMin, $lte: valueMax };
    } else if (queryparam["Valeur fonciere max"]) {
      const value = parseInt(queryparam["Valeur fonciere max"]);
      query["Valeur fonciere"] = { $lte: value };
    } else if (queryparam["Valeur fonciere min"]) {
      const value = parseInt(queryparam["Valeur fonciere min"]);
    ...
    if (queryparam["Surface min"] && queryparam["Surface max"]) {
      const surfaceMin = parseInt(queryparam["Surface min"]);
      const surfaceMax = parseInt(queryparam["Surface max"]);
      query["Surface reelle bati"] = { $gte: surfaceMin, $lte: surfaceMax };
    } else if (queryparam["Surface min"]) {
      const surfaceMin = parseInt(queryparam["Surface min"]);
      query["Surface reelle bati"] = { $gte: surfaceMin };
    } else if (queryparam["Surface max"]) {
      const surfaceMax = parseInt(queryparam["Surface max"]);
      query["Surface reelle bati"] = { $lte: surfaceMax };
    }
    ...
    if (queryparam["Nombre pieces principales"]) {
      ...
      if (nbPieces === 5) {
        ...
        query["Nombre pieces principales"] = nbPieces;
      }
    }
